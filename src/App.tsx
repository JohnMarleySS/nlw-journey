...
export function App() {
  ...
  const [emailsToInvite, setEmailsToInvite] = useState([
    "johnmarley@outlook.com.br", "johnmarley2001outlook.com.br"
  ])

  function openGuestOpen() {
    ...
    <div className="h-screen flex items-center justify-center bg-pattern bg-no-repeat bg-center">
      <div className="max-w-3xl w-full px-6 text-center space-y-10">
        <div className="flex flex-col items-center gap-3">
          <img src="/logo.svg" alt="planner" className="w-44" />
        ...
        <div className=" space-y-4">
          <div className="h-16 bg-zinc-900 px-4 rounded-xl flex items-center shadow-shape gap-3">
            <div className=" flex items-center gap-2 flex-1">
              ...
              <input disabled={isGestsInputOpen} type="text" placeholder="Para onde você vai?" className="bg-transparent text-lg placeholder-zinc-400 outline-none" />
            </div>
            ...
            {isGestsInputOpen ? (
              ...
              <button onClick={openGuestOpen} className=" bg-lime-300 text-lime-950 rounded-lg px-5 py-2 font-medium flex items-center gap-2 hover:bg-lime-400">
                Continuar
          ...
          <div className=" w-[640px] rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5">
            ...
            <div className=" flex flex-wrap gap-2">
              {emailsToInvite.map(email => {
                return (
                  <div key={email} className=" py-1.5 px-2.5 bg-zinc-800 flex items-center gap-2">
                    <span> {email}</span>
                    <button onClick={() => removeEmailFromEnvites(email)}>
                      <X className="size-4 text-zinc-400" type="button" />
                    </button>
                  </div>
                )
              })}
